...
import net.xdob.ratly.fasts.offheap.structs.FSTStruct;
        ...
        try {
            ...
            } else
            {
                ...
                "{"+
                    ...
                    "structs.offheap.net.xdob.ratly.fasts.FSTStruct struct = (structs.offheap.net.xdob.ratly.fasts.FSTStruct)$1;"+
                    "if ( !struct.isOffHeap() ) {"+
...
//                        "unsafe.copyMemory(struct.___bytes,struct.___offset,___bytes,(long)_st_off+$1*_elem_len,(long)struct.getByteSize());"+
    ...
    public void defineArrayPointer(FSTClazzInfo.FSTFieldInfo indexfi, FSTClazzInfo clInfo, CtMethod method) {
        ...
        if ( parameterTypes != null && parameterTypes.length ==1 ) {
            ...
            try {
                if (indexfi.isIntegral()) {
                    method.setBody("{ return (structs.offheap.net.xdob.ratly.fasts.FSTStruct)___fac.createPrimitiveArrayBasePointer(___bytes, ___offset, "+index+"); }");
                } else
        ...
        try {
            ...
            if ( type == CtPrimitiveType.doubleType ) {
                ...
                statement = "{ int tmpIdx = ___bytes.getInt( " + off + " + ___offset); if (tmpIdx < 0) return null;" +
                        ...
                        (trackChanges ? "tmp.tracker = new structs.offheap.net.xdob.ratly.fasts.FSTStructChange(tracker,\"" + fieldInfo.getName() + "\"); " : "")+
                        "$_ = tmp; " +
                        "}";
                f.replace(statement);
