...
public class Meta extends SQLClient {
  ...
  protected void operation(RaftClient client) throws IOException {
    ...
    if(!queryReplyProto.hasEx()) {
      ...
      try {
        SerialResultSetMetaData rsMeta = (SerialResultSetMetaData) fst.asObject(queryReplyProto.getRsMeta().toByteArray());

        for (int i = 1; i <= rsMeta.getColumnCount(); i++) {
          LOG.info("{} name={} type={}", i, rsMeta.getColumnName(i), rsMeta.getColumnTypeName(i));
        }

        SerialParameterMetaData paramMeta = (SerialParameterMetaData) fst.asObject(queryReplyProto.getParamMeta().toByteArray());
